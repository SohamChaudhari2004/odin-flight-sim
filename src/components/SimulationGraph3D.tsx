...
function Sphere3D({ position, color, label, size = 0.2 }: { 
...
export default function SimulationGraph3D({ activeTrajectory = 'baseline' }: SimulationGraph3DProps) {
  ...
  const dataPoints: DataPoint[] = useMemo(() => {
    return trajectories.map(traj => {
      ...
      const color = traj.risk === 'Low' ? '#00FF00' : 
                   traj.risk === 'High' ? '#FF4500' : '#FFFF00';
      ...
      <CardContent className="h-[calc(100%-60px)] p-0">
        <div className="w-full h-full bg-gradient-cosmic relative">
          ...
          <div className="absolute top-4 right-4 bg-card/90 backdrop-blur-sm border border-border rounded-lg p-3">
            <h4 className="text-xs font-mono-mission text-primary mb-2">LEGEND</h4>
          ...
          <div className="absolute bottom-4 left-4 bg-card/90 backdrop-blur-sm border border-border rounded-lg p-3">
            <h4 className="text-xs font-mono-mission text-primary mb-2">ACTIVE TRAJECTORY</h4>
            {(() => {
              const active = trajectories.find(t => t.id === activeTrajectory);
              return active ? (
                <div className="space-y-1">
                  <div className="text-xs text-muted-foreground">
                    <span className="text-warning">ΔV:</span> {active.deltaV} m/s
                  </div>
                  <div className="text-xs text-muted-foreground">
                    <span className="text-accent">Time:</span> {active.travelTime}h
                  </div>
                  <div className="text-xs text-muted-foreground">
                    <span className="text-destructive">Radiation:</span> {active.radiationExposure}%
                  </div>
                </div>
              ) : null;
            })()}
